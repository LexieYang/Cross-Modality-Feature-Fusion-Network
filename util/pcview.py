...
def distribute(depth, _x, _y, size_x, size_y, image_height, image_width):
    ...
    _i = torch.linspace(-size_x / 2, (size_x / 2) - 1, size_x, requires_grad=False, device=depth.device) #[-0.5]
    _j = torch.linspace(-size_y / 2, (size_y / 2) - 1, size_y, requires_grad=False, device=depth.device) #[-0.5]
...
if __name__=='__main__':
    ...
    points=np.load('./scanobjectnn/ScanObjectNN_fs_cross_validation/Data/14/9.npy')   
    ...
    for img in imgs:
        ...
        cv2.imwrite('m40_19_'+str(i)+'.png',img)
        i+=1
        # cv2.waitKey(0)
